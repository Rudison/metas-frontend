...
export default {
  ...
  methods: {
    cardSelecionado(card) {
      if (!card.popup) this.$emit("tituloSelecionado", card.titulo, card.icon);

      this.$emit("iconSelecionado", card.icon);

      this.inicio = false;

      if (card.filhos) this.cards = card.filhos;
    ...
    showModal(popup) {
      const componente =
        this.componente == "Vendedores"
          ? Vendedores
          : this.componente == "Feriados"
          ? Feriados
          : "";
